function carregar_jogo() {
    if (file_exists(global.save_file)) {
        ...
        if (buffer_tell(buffer) + 4 <= tamanho) global.musica = buffer_read(buffer, buffer_u32); else global.musica = 0;
        if (buffer_tell(buffer) + 4 <= tamanho) global.efeitos = buffer_read(buffer, buffer_u32); else global.efeitos = 0;
